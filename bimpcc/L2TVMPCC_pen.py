...
class L2TVMPCC_pen(AbstractMPCC):
    ...
    def __init__(self, pi, A, Kx, Ky, Q, utrue, unoisy, epsilon=1e-4, gamma=0):
        ...
        pi: float
            Penalty parameter
        tau: float
            Penalty parameter (positiveness of Q@alpha - delta)
    ...
    def objective(self, x):
        ...
        return 0.5*np.linalg.norm(u-self.utrue)**2 + self.pi*self.complementarity(x) + self.pi*(np.sum(np.log(self.Q@alpha-delta))) + self.epsilon*np.linalg.norm(qx)**2 + self.epsilon*np.linalg.norm(qy)**2 + self.epsilon*np.linalg.norm(alpha)**2 + self.epsilon*np.linalg.norm(r)**2 + self.epsilon*np.linalg.norm(theta)**2 + self.epsilon*np.linalg.norm(delta)**2

    def gradient(self, x):
        u, qx, qy, alpha, r, delta, theta = self.getvars(x)
        den = (1/np.reshape(self.Q@alpha-delta, (self.M, self.P)))
        grad_u = u-self.utrue
        grad_qx = np.zeros(self.M) + 2*self.epsilon*qx
        grad_qy = np.zeros(self.M) + 2*self.epsilon*qy
        grad_alpha = self.pi*(self.Q.T@r) + 2*self.epsilon*alpha + self.pi*den.T@self.Q
        ...
        print(f'{grad_alpha=}\n{grad_alpha.shape=}')
        return np.concatenate((grad_u, grad_qx, grad_qy, grad_alpha.ravel(), grad_r, grad_delta, grad_theta))
