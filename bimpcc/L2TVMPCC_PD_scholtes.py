...
class L2TVMPCC_PD_scholtes(AbstractMPCC):
    ...
    def objective(self, x):
        u, qx, qy, alpha, r, delta, theta = self.getvars(x)
        return 0.5*np.linalg.norm(u-self.utrue)**2 + self.epsilon*np.linalg.norm(qx)**2 + self.epsilon*np.linalg.norm(qy)**2 + self.epsilon*np.linalg.norm(alpha)**2 + self.epsilon*np.linalg.norm(r)**2 + self.epsilon*np.linalg.norm(delta)**2
    ...
    def gradient(self, x):
        ...
        grad_alpha = 2*self.epsilon*alpha
        grad_r = 2*self.epsilon*r
        grad_delta = 2*self.epsilon*delta
    ...
    def jacobianstructure(self):
        jac = bmat([
            ...
            [None,None,None,self.Q,self.I,self.I,None]
        ])
    ...
    def jacobian(self, x):
        ...
        jac = bmat([
            ...
            [None,None,None,self.Q,None,-self.I,None],
            [None, None, None, -diags(r)@self.Q, -diags(self.Q@alpha-delta), diags(r), None]
        ])
    ...
    def hessian(self, x, lagrange, obj_factor):
        ...
        hess = bmat([
            ...
            [None,None,None,self.Q,D2,None,None],
            [None,None,None,None,-self.I,D2,None],
